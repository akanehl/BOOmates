...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class FindClosestTask : FsmStateAction
	{
		[RequiredField]
		...
		void DoFindClosest()
		{
			...
			if (string.IsNullOrEmpty(withTag.Value) || string.IsNullOrEmpty(withTag2.Value)|| withTag.Value == "Untagged")
			{
				objects = (GameObject[])GameObject.FindObjectsOfType(typeof(GameObject));
			}
			else
			{
				GameObject[] objects1 = GameObject.FindGameObjectsWithTag(withTag.Value);
				GameObject[] objects2 = GameObject.FindGameObjectsWithTag(withTag2.Value);
				objects = objects1.Concat(objects2).ToArray();
			}	
