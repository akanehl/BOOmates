...
{
    [TaskDescription("Synchronize a SharedQuaternion to/from a FsmQuaternion.")]
    ...
    public class SynchronizeQuaternion : Action
    {
        ...
        public SharedQuaternion sharedVariable = Quaternion.identity;
        ...
        {
            if (isGlobalFsmVariable.Value) {
                var fsmVariable = FsmVariables.GlobalVariables.FindFsmQuaternion(FsmVariableName.Value);
                ...
                if (playMakerFSM != null) {
                    var fsmVariable = playMakerFSM.FsmVariables.FindFsmQuaternion(FsmVariableName.Value);
                    // The variable has to exist
                    if (fsmVariable != null) {
                        // Sync to Behavior Designer
                        if (toBehaviorDesigner.Value) {
        ...
        public override void OnReset()
        {
            ...
            sharedVariable = Quaternion.identity;
