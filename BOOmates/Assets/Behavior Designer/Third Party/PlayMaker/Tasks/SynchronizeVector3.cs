...
{
    [TaskDescription("Synchronize a SharedVector3 to/from a FsmVector3.")]
    ...
    public class SynchronizeVector3 : Action
    {
        ...
        {
            if (isGlobalFsmVariable.Value) {
                var fsmVariable = FsmVariables.GlobalVariables.FindFsmVector3(FsmVariableName.Value);
                ...
                if (playMakerFSM != null) {
                    var fsmVariable = playMakerFSM.FsmVariables.FindFsmVector3(FsmVariableName.Value);
                    // The variable has to exist
                    if (fsmVariable != null) {
                        // Sync to Behavior Designer
                        if (toBehaviorDesigner.Value) {
