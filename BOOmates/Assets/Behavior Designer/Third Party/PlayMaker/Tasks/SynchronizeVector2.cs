...
{
    [TaskDescription("Synchronize a SharedVector2 to/from a FsmVector2.")]
    ...
    public class SynchronizeVector2 : Action
    {
        ...
        {
            if (isGlobalFsmVariable.Value) {
                var fsmVariable = FsmVariables.GlobalVariables.FindFsmVector2(FsmVariableName.Value);
                ...
                if (playMakerFSM != null) {
                    var fsmVariable = playMakerFSM.FsmVariables.FindFsmVector2(FsmVariableName.Value);
                    // The variable has to exist
                    if (fsmVariable != null) {
                        // Sync to Behavior Designer
                        if (toBehaviorDesigner.Value) {
